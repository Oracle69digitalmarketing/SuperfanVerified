...
export const refresh = async (req, res) => {
  try {
    ...
    if (!stored || stored.revoked) return res.status(401).json({ success: false, message: 'Invalid refresh token' });
    if (new Date() > stored.expiresAt) return res.status(401).json({ success: false, message: 'Expired refresh token' });

    // rotate: create new refresh token, revoke old
    const newRefreshValue = uuidv4();
    const newExpires = new Date(Date.now() + daysToMs(parseInt(process.env.REFRESH_TOKEN_EXPIRES_DAYS || '30')));

    stored.revoked = true;
    stored.replacedBy = newRefreshValue;
    await stored.save();

    const newStored = await RefreshToken.create({ token: newRefreshValue, user: stored.user._id, expiresAt: newExpires });

    const accessToken = signAccessToken({ sub: stored.user._id, provider: stored.user.provider });

    return res.json({ success: true, accessToken, refreshToken: newStored.token, expiresAt: newStored.expiresAt });
  } catch (err) {
